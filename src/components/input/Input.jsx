...
function Input({ label, placeHolder, errorMessage, type, inputId, InoputFunction, value, disable, clipboard }) {
    const types = ["checkbox", "range"].includes(type)
    const toast = useToast()
    const handleClipboard = (value) => {
        if (value) {
            navigator.clipboard.writeText(value)
            return toast.open({ type: 'Success', content: "Item copied to clipboard" })
        }
    ...
    return (
        <div className={`${types ? 'mt-5 flex' : 'mt-5'}`}>
            ...
            <div className='flex items-center'>
                <input onChange={InoputFunction} type={type} value={value} id={inputId} min={types ? "6" : ''} max={types ? "20" : ''} defaultValue={type ? 0 : ''} step={types ? 0 : ''} className={!types ? `bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg
                ...
                {
                    clipboard && <svg onClick={() => handleClipboard(value)} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6 ml-3 cursor-pointer ">
